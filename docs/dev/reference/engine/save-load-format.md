...
> **Status:** reference   ·   **Verified against:** `Sources/Infrastructure/CvTaggedSaveFormatWrapper.{h,cpp}` (path confirmed 2026-06-19; format mechanics authored in the #428 feat commit)
> **Grounding:** `Sources/Infrastructure/CvTaggedSaveFormatWrapper.{h,cpp}` (line numbers below are from there unless noted; `CvGlobals.cpp` for the name→int registry).
> How S2S serializes game state, and — the practically important part — **what actually happens when you
> remove a serialized field, member, or Type.** Bottom line up front: **the format is name-keyed, so
> removing a plain serialized member is a SOFT change, not a save-break.** This doc is the authoritative
> home of [DEC-save-remove-is-soft](../../architecture/decisions.md#dec-save-remove-is-soft) and keeps that
> rule's full explanation.

> **Citations drift.** Line numbers below are "the function named here, around this line" — confirm the
> **function**, not the integer. The class moved from `Sources/` to `Sources/Infrastructure/` since the
> original authoring; the cited functions (`getNewClassEnumValue`, `ReadEndObject`, `SkipElement`,
> `ReadClassArray`, `HandleIncompatibleSave`, `Expect`) were re-confirmed present at the cited
> neighbourhood on 2026-06-19.

> **Branch-resident flag (carried from the old doc):** the **format mechanics below are
> stable/authoritative.** The only part still being finalized is the **cascade-specific** save handling —
> what the #428/#430 tally and accumulators serialize (answer: nothing; rebuilt on load), captured by
> [DEC-tally-serializes-nothing](../../architecture/decisions.md#dec-tally-serializes-nothing). That work
> is branch-resident on `json-data-migration` pending an owner call on promoting it back to `main`.
...
| **A Type inside a class-array** (`m_ab*[BUILDING_X]`) | **SOFT (best-effort)** — `ReadClassArray` (`3415-3470`) drops vanished slots; only a *non-default* value at a gone slot raises a **recoverable** warning, not a crash; old/new counts need not match | `3438-3470` |
...
### DEC-save-remove-is-soft — removing a serialized accumulator is SOFT, not a save-break

Deleting the `WRAPPER_WRITE`/`WRAPPER_READ` pair for a plain accumulator costs **nothing** on load: old saves
carry the orphan tag, the new read never asks for it, the skip loop eats it. The only "loss" is historical
bytes nobody reads. ("Structural mid-serialization desync" is **not** a real failure mode of the symmetric
same-era format — the name-keyed `Expect` + delimiter-drain design prevents it; it would only bite on a *type-code
change under a reused name*.)
...
## Derived data shrinks the problem

Derived data **serializes nothing** — every owner's `reset()` marks derived data dirty on load and recomputes
it from live state ([DEC-derived-never-trusted](../../architecture/decisions.md#dec-derived-never-trusted)).
So **derived state has nothing to migrate.** If an accumulator's only job is to feed a derived value, deleting
its serialization is the *intended* pattern — the value recomputes on load. The #423 recompute-on-load cascade
generalizes exactly this: **the more authoritative state becomes derived, the smaller the serialized surface,
and the less there is to break or migrate.**

This is the stance the **#428/#430 cascade** takes: the tally and scope accumulators serialize nothing —
rebuilt from the authoritative loaded objects on load
([DEC-tally-serializes-nothing](../../architecture/decisions.md#dec-tally-serializes-nothing)). Anything
genuinely NOT recomputable from current state (a true lifetime/historical counter) is owned + saved on its
**owning object**, never in a derived/aggregation module — so a derived module is always safely rebuildable and
nothing unrecoverable lives only in it.
...
- [Decisions ledger](../../architecture/decisions.md) — the canonical ID'd home for cross-cutting rulings;
  this doc owns [DEC-save-remove-is-soft](../../architecture/decisions.md#dec-save-remove-is-soft) and links
  out for [DEC-tally-serializes-nothing](../../architecture/decisions.md#dec-tally-serializes-nothing) and
  [DEC-derived-never-trusted](../../architecture/decisions.md#dec-derived-never-trusted).
- [Cascade architecture](../../explanation/cascade-architecture.md) — *why* the tally/accumulators are
  derived (and therefore serialize nothing), which is what makes the cascade's serialized surface tiny.
- [Comprehension map](../../README.md) — the overview-of-overviews; start here to place this engine doc among
  the other subsystems.
- [`AGENTS.md`](../../../../AGENTS.md) — the save-compatibility convention (`@SAVEBREAK`, front-load save-breaks
  now) that this doc's soft/hard classification informs.
